...
<head>
	...
	<script type="text/javascript">	
		$(function(){
			...
			$('#dlg').dialog('close');
			$('#dlg2').dialog('close');	
			$("#dg").datagrid('options').url ='/ddxq/employees/info/getinfo';
...
<body style="font-family: verdana, helvetica, arial, sans-serif;
    ...
    margin-right: 2px;
    margin-left: 2px;">
	<table id="dg" class="easyui-datagrid" title="我的信息"  fit="true"
			data-options="
				iconCls: 'icon-edit',
				singleSelect: true,
				rownumbers:true,
	...
	<div id="tb" style="height:auto">	
		<select class="easyui-combobox" id="selectactor" disabled="disabled" label="我的身份：" style="width:25%">
...
<script type="text/javascript">
	...
	<div id="dlg2" class="easyui-dialog" title="修改个人信息" data-options="iconCls:'icon-save'" style="width:450px;height: 450px;padding:5px">	
		<form id="ff2" method="post">
			...
			<div style="margin-bottom:10px">
				<select class="easyui-combobox" id="employeeActor2" label="选择管理用户类型" style="width:90%">
					<option value="700">骑手</option><option value="800">商家</option><option value="200">物业</option><option value="500">业委会</option><option value="170">快递代收</option>
					<option value="300">居委会</option><option value="110">片警</option><option value="120">家庭医生</option><option value="150">小区中介</option><option value="400">客服</option>
				</select>
			</div>
	...
	<script> 
		function submitEdit(){
			...
			$.ajax({
				url : '/ddxq/employees/info/editInfo',
				data : $.toJSON(retData),
				type : 'post',
				dataType : 'json',
