...
<head>
	...
	<script type="text/javascript">	
		$(function(){
			$('#dlg').dialog('close');
			$('#dlg2').dialog('close');	
			$("#dg").datagrid('options').url ='/ddxq/admin/userman/show?actorid='+${actorid};
			$("#selectactor").combobox({ disabled: true});
			$("#selectactor").combobox('setValue',${actorid});
			$("#employeeActor").combobox({ disabled: true});
			$("#employeeActor").combobox('setValue',${actorid});
			$("#employeeActor2").combobox({ disabled: true});
			$("#employeeActor2").combobox('setValue',${actorid});
	    	$("#dg").datagrid('reload');
...
<body    style="font-family: verdana, helvetica, arial, sans-serif;
    ...
    margin-left: 2px;
    " >
	<table id="dg" class="easyui-datagrid" title="员工信息" fit="true"  border="true" fitcolums="true"
			data-options="
				iconCls: 'icon-edit',
				singleSelect: true,
				rownumbers:true,
		...
		<thead>
			<tr>
				<th data-options="field:'employeeId',width:80">账号（手机号）</th>
				...
				<th data-options="field:'id',width:0,hidden:true,align:'center'">id</th>
			</tr>
		</thead>
	</table>
	
	<div id="tb" style="height:auto">		
		<select class="easyui-combobox"id="selectactor" label="员工类型:" style="width:25%">
			<option value="700">骑手</option><option value="800">商家</option><option value="200">物业</option><option value="500">业委会</option>
			<option value="170">快递代收</option><option value="300">居委会</option><option value="110">片警</option><option value="120">家庭医生(暂无)</option><option value="150">中介(暂无)</option><option value="400">客服(暂无)</option>
		</select>
...
<script type="text/javascript">
		...
		function getData2(){
			...
			retData.employeeNation=$("#employeeNation2").textbox('getValue');
		...
		function append(){
			$('#dlg').dialog('open');
			$("#employeeActor").combobox({ disabled: true});
			$("#employeeActor").combobox('setValue',$("#actor").val());
			$("#districtId").numberbox('setValue',$("#district").val());
			$("#districtId").numberbox({ disabled: true});
		}
		function removeSelect(){
			var r=confirm("是否删除选中的用户？");
			if (r==false){
				 return false;
			 }
			var retData={};
...
<div id="dlg" class="easyui-dialog" title="添加员工" data-options="iconCls:'icon-save'" style="width:450px;height: 350px;padding:5px">	
		<form id="ff" method="post">
			...
			<div style="margin-bottom:10px"> 
				<select class="easyui-combobox"id="employeeActor" label="选择员工类型" style="width:90%">
					<option value="700">骑手</option><option value="800">商家</option><option value="200">物业</option><option value="500">业委会</option>
					<option value="170">快递代收</option><option value="300">居委会</option><option value="110">片警</option><option value="120">家庭医生(暂无)</option><option value="150">中介(暂无)</option><option value="400">客服(暂无)</option>
				</select>
			</div>
			...
			<div style="margin-bottom:10px">
				<input class="easyui-textbox" id="employeeNation" name="employeeNation" style="width:90%" data-options="label:'民族:',required:true">
			</div>				
<!-- 			<div style="margin-bottom:10px">
				<input class="easyui-numberbox" id="employeeMobile"name="employeeMobile" style="width:90%" data-options="label:'手机号:',required:true">
			</div> -->
			<div style="margin-bottom:10px">
			...
			<div style="margin-bottom:10px">
				<input class="easyui-textbox" id="employeeAddr" name="employeeAddr" style="width:90%" data-options="label:'地址:',required:true">
			</div>
			<div style="margin-bottom:10px">
		...
		function getData(){
			...
			retData.employeeNation=$("#employeeNation").textbox('getValue');
		...
		function submitForm(){
			var retData=getData();
			$.ajax({
				url : '/ddxq/admin/userman/insertInfo',
				data : $.toJSON(retData),
				type : 'post',
				dataType : 'json',
				contentType : 'application/json',
	...
	<div id="dlg2" class="easyui-dialog" title="修改员工信息" data-options="iconCls:'icon-save'" style="width:450px;height: 350px;padding:5px">	
		<form id="ff2" method="post">
			...
			<div style="margin-bottom:10px">
				<select class="easyui-combobox"id="employeeActor2" label="管理员类型" style="width:90%">
					<option value="700">骑手</option><option value="800">商家</option><option value="200">物业</option><option value="500">业委会</option>
					<option value="170">快递代收</option><option value="300">居委会</option><option value="110">片警</option><option value="120">家庭医生(暂无)</option><option value="150">中介(暂无)</option><option value="400">客服(暂无)</option>
				</select>
			</div>
			...
			<div style="margin-bottom:10px">
				<input class="easyui-textbox" id="employeeNation2"name="employeeNation" style="width:90%" data-options="label:'民族:',required:true">
			</div>			
<!-- 			<div style="margin-bottom:10px">
	...
	<input type="hidden" id="actor" name="actor" value="${actorid}" />
	<input type="hidden" id="district" name="district" value="${districtId}" />
</body>
</html>