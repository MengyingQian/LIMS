...
<div style="padding:10% 20%;"> 
	 ...
	 <c:if test="${type eq 3 }">
	  ...
	  <c:if test="${type eq 4 }">
	 		<div class="easyui-panel" title="设置房屋租售状态" style="width:100%;max-width:1000px;padding:20px 20px;">
	 </c:if> 
	   <c:if test="${type eq 5 }">
	 		<div class="easyui-panel" title="设置招聘求职状态" style="width:100%;max-width:1000px;padding:20px 20px;">
	 </c:if> 	 
...
function open1(){
	...
	if (type == "4"){
		var r=confirm("开启前请确保房屋租售服务准备妥当，已经确认？");
		if (r==false){
			 return false;
		 }
	}
	if (type == "5"){
		var r=confirm("开启前请确保招聘求职服务准备妥当，已经确认？");
		if (r==false){
			 return false;
		 }
	}
