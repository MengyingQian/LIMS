...
<body style="font-family: verdana, helvetica, arial, sans-serif;
	...
	<div id="tb" style="height:auto">
		...
		选择消息类型： <select id="selectnews" disabled="disabled" style="height:25px;width: 15%;margin-left: 5px;border-radius: 3px;">
			<option value="wytz">物业通知</option><option value="xmqs">寻觅启事</option><option value="sqgg">社区公告</option><option value="xqhd">小区活动</option><option value="stzz">社团组织</option><option value="yewh">业委会</option><option value="juwh">居委会</option><option value="pchs">派出所</option>
		</select>
	...
	<div id="dlg" class="easyui-dialog" title="添加消息" data-options="iconCls:'icon-save'" style="width:450px;height: 420px;padding:5px">	
		<form id="ff" method="post">
			...
			消息类型：
				<select id="info" disabled="disabled" name="info" style="width: 69%;margin-left: 25px;border-radius: 3px;"><option value="wytz">物业通知</option><option value="xmqs">寻觅启事</option><option value="sqgg">社区公告</option><option value="xqhd">小区活动</option><option value="stzz">社团组织</option><option value="yewh">业委会</option><option value="juwh">居委会</option><option value="pchs">派出所</option></select>
			</div>
			<div style="margin-bottom:10px">
			...
			$.ajax({
				...
				success : function(data) {
					if(data.success==true){
						...
						$("#selectnews").val(data.info);
						$("#dg").datagrid('options').url = data.url;
						$("#dg").datagrid('reload');					
					}else{
						alert("插入失败，单个类型的消息每天最多添加3条");
					}				
				},
				error : function(xhr) {
				}
  ...
  <input type="hidden" id="h8" name="subtitle" value="" />  
	...
	<div id="dlg2" class="easyui-dialog" title="添加消息" data-options="iconCls:'icon-save'" style="width:450px;height: 420px;padding:5px">	
		<form id="ff2" method="post">
			...
			<div style="margin-bottom:10px">
			消息类型：
				<select id="info2" disabled="disabled" name="info"  style="width: 69%;margin-left: 25px;border-radius: 3px;"><option value="wytz">物业通知</option><option value="xmqs">寻觅启事</option><option value="sqgg">社区公告</option><option value="xqhd">小区活动</option><option value="stzz">社团组织</option><option value="yewh">业委会</option><option value="juwh">居委会</option><option value="pchs">派出所</option>
				</select>
			</div>
			<div style="margin-bottom:10px">
	...
	<input type="hidden" id="messtype"name="messtype" value="${messtype}">
</body>
</html>