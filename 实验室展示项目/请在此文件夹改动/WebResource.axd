$(function() {
    window.modalLoading = function() {
        ...
        var n = $(".modal:first");
        return n.find(".modal-title").text("loading..."),
        n.find(".modal-body").html("").append($("<div>").addClass("loading").html("正在努力加载中...")),
    ...
    window.delSelectedCallback = function(n) {
        if (n && n.result && n.dataid) {
            closeModal();
            for (var t = 0; t < n.dataid.length; t++)
                $('[data-id="' + n.dataid[t] + '"]').fadeOut("slow", function() {
                    $(this).remove()
                })
        } else
            n.message && alert(n.message)
    ...
    window.modalForm = function(n) {
        ...
        i.removeData("unobtrusiveValidation"),
